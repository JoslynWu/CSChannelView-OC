...
- (void)loadDataInfoWithImages:(nullable NSArray<NSString *> *)imageGroup tittls:(nullable NSArray<NSString *> *)titleGroup tagImages:(nullable NSArray<NSString *> *)tagImageGroup {
    ...
    [self refreshTypeWithImages:imageGroup tittls:titleGroup tagImages:tagImageGroup];
    [self dealDataWithImages:imageGroup tittls:titleGroup tagImages:tagImageGroup];
...
#pragma mark  -  action
- (void)refreshTypeWithImages:(nullable NSArray<NSString *> *)imageGroup tittls:(nullable NSArray<NSString *> *)titleGroup tagImages:(nullable NSArray<NSString *> *)tagImageGroup {
    CSChannelType type = CSChannelTypeNormal;
    if (titleGroup.count <= 0) {
        if (imageGroup.count <= 0) {
            type = CSChannelTypeNone;
        } else {
...
- (void)dealDataWithImages:(nullable NSArray<NSString *> *)imageGroup tittls:(nullable NSArray<NSString *> *)titleGroup tagImages:(nullable NSArray<NSString *> *)tagImageGroup {
...
- (void)refreshUI {
    ...
    self.pageCount = (self.autoAdjustHeight || self.verticalScrollActivated ? 1 : ceil(itemCount / (self.numberOfItemInRow * self.numberOfRowInPage + 0.1)));

    self.pageArr = [NSMutableArray arrayWithCapacity:self.pageCount];
...
- (void)refreshFrame {
    ...
    CGFloat contentSize_h = (self.verticalScrollActivated ? page_h_max : scrollView_h);
    self.scrollView.contentSize = CGSizeMake(self.pageCount * page_w, contentSize_h);
    
    for (int i = 0 ; i < self.scrollView.subviews.count; i++) {
        UIView *pageView = self.scrollView.subviews[i];
        pageView.frame = CGRectMake(i * scrollView_w, 0, scrollView_w, contentSize_h);
    }
...
- (void)refreshPageCtl {
    ...
    self.pageCtl.numberOfPages = self.pageCount;
...
- (CGSize)calcTitleSizeWithTitle:(NSString *)title {
    ...
    CGFloat title_h_max = (page_h - imge_total_h - self.distanceOfTop - self.distanceOfbottom - self.distanceOfRow * (self.numberOfRowInPage - 1)) / self.numberOfRowInPage;
